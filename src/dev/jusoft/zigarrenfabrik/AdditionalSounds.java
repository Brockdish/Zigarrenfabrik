...
public class AdditionalSounds implements Listener {
    ...
    void checkIfValidValuesInConfigForJoinLeaveSound() {
    ...
    void playJoinLeaveSound(boolean joined) {
        ...
        checkIfValidValuesInConfigForJoinLeaveSound();

        noteIndex = 0;
        pitch = 0;
        Bukkit.getScheduler().cancelTask(scheduler);
        scheduler = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
    ...
    @EventHandler void onPlayerArrowHitEntity(EntityDamageByEntityEvent event) {
        ...
        if (!(event.getEntity() instanceof LivingEntity) || event.getFinalDamage() == 0) return;
